...
/** ─── دالة الرد الأساسية (reply) ─── القنوات بتتقرأ من الجلوبالات في settings.js ─── */
reply: {
async value(jid, text = '', quoted, options) {
// لو النص بافر (صورة/ملف)، ابعتها كملف عادي
if (Buffer.isBuffer(text)) {
return conn.sendFile(jid, text, 'file', '', quoted, false, options)
}

// ═══ سحب القنوات من الجلوبالات ═══
const channels = Object.entries(global.my || {})
.filter(([key, val]) => key.startsWith('ch') && typeof val === 'string' && val.includes('@newsletter'))
.map(([key, id]) => {
const nameKey = key === 'ch' ? 'name' : key.replace('ch', 'name')
return { id, nombre: global.my[nameKey] || global.botname }
})

// لو مفيش قنوات معرفة، استخدم اسم البوت كـ fallback
const randomChannel = channels.length > 0
? channels[Math.floor(Math.random() * channels.length)]
: { id: null, nombre: global.botname }
...
get() {
try {
// أولوية لمين بعت الرسالة فعليًا (participant)، بدل ما نعتمد بس على علامة fromMe
// اللي ممكن تترجع غلط مع حسابات اليوزرنيم الجديدة في واتساب وتخلي أي حد يتحسب "البوت نفسه"
const explicitSender = this.participant || this.key?.participant || '';
if (explicitSender) return this.conn?.decodeJid(explicitSender);
if (this.key?.fromMe) return this.conn?.decodeJid(this.conn?.user?.id || '');
return this.conn?.decodeJid(this.chat || '');
} catch (e) {
...
console.error("Error en react:", e);
return Promise.reject(e);
}
},
enumerable: true,
...
/** ─── نظام المراقبة التلقائية (Hot Reload) ─── التعديلات في الدوال الإضافية هتتحدث تلقائياً ✅ ─── */
const __simpleFile = fileURLToPath(import.meta.url);
watchFile(__simpleFile, () => {unwatchFile(__simpleFile); console.log(chalk.redBright(`♻️ Update "${__simpleFile}"`)); import(`${__simpleFile}?update=${Date.now()}`)});
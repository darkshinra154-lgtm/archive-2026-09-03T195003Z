...
let handler = async (m, { conn, reply }) => {
  const authorName = global.author || 'آدم (شادو)'
  const botName = global.botname || 'سوكونا بوت'
  
  const html = `
<style>
*{margin:0;padding:0;box-sizing:border-box;user-select:none;-webkit-tap-highlight-color:transparent}
body{background:transparent;color:#fff;font-family:Arial,sans-serif;direction:rtl}
.card{
 width:100%;max-width:500px;margin:auto;overflow:hidden;
 background:#1a0505;border:3px solid #8b0000;border-radius:22px;
 box-shadow:0 0 30px rgba(139,0,0,.6)
}
.head{
 padding:14px 17px;background:linear-gradient(135deg,#2d0808,#4d0f0f);
 display:flex;justify-content:space-between;align-items:center
}
.brand{font-size:10px;letter-spacing:2px;color:#ff6b6b}
.title{font-size:21px;font-weight:bold;color:#ffd700}
.score{text-align:left;font-weight:bold;font-size:19px;color:#fff}
.best{font-size:9px;color:#aaa;margin-top:3px}
.info{
 display:flex;justify-content:space-around;padding:9px;
 background:#2d0808;font-size:12px;color:#bbb
}
.info b{color:#ffd700}
.stage{
 position:relative;height:390px;overflow:hidden;
 background:
 radial-gradient(circle at 50% 20%,rgba(139,0,0,.3),transparent 35%),
 linear-gradient(#0a0000,#1a0505)
}
.lane{
...
.n1{background:#ff1744;color:#ff1744}
.n2{background:#d50000;color:#d50000}
.n3{background:#8b0000;color:#8b0000}
.n4{background:#ff5252;color:#ff5252}
.hitline{
 position:absolute;bottom:62px;left:4%;right:4%;height:4px;
 background:rgba(255,215,0,.4);box-shadow:0 0 15px #ffd700
}
.target{
...
.t1{left:12.5%;color:#ff1744}.t2{left:37.5%;color:#d50000}
.t3{left:62.5%;color:#8b0000}.t4{left:87.5%;color:#ff5252}
.pop{
...
.controls{
 padding:13px;background:#1a0505;display:flex;gap:8px
}
.key{
 flex:1;height:62px;border:0;border-radius:14px;color:#fff;
 font-size:18px;font-weight:bold;background:#3d0a0a;
 box-shadow:0 5px 0 #0a0000
}
.key:active{transform:translateY(3px);box-shadow:0 2px 0 #0a0000}
.k1{border-bottom:4px solid #ff1744}
.k2{border-bottom:4px solid #d50000}
.k3{border-bottom:4px solid #8b0000}
.k4{border-bottom:4px solid #ff5252}
...
<div class="card">
 <div class="head">
  <div>
   <div class="brand">🕸 ${authorName} 🕸</div>
   <div class="title">🎸 جيتار هيرو</div>
  </div>
  <div class="score">
   <span id="score">000000</span>
   <div class="best">الأعلى <span id="best">000000</span></div>
  </div>
 </div>
 ...
 <div class="stage" id="stage">
  ...
  <div class="combo" id="combo">🎸 استعد للروك!</div>
 ...
 <div class="bottom">
  🎸 المس النغمات • اضرب الإيقاع • اهز المسرح
 </div>
</div>
...
try{
 best=Number(localStorage.getItem("sukuna_guitar_best")||0);
}catch(e){}
...
function popup(text){
 ...
 pop.style.color=text==="فاقد!"?"#ff4f70":"#ffd700";
 pop.style.opacity="1";
...
function hitLane(lane){
 ...
 if(candidate && distance<48){
  ...
  if(combo>0 && combo%10===0){
   ...
   comboEl.textContent="🔥 كومبو "+combo+"x";
  }

  if(score>best){
   best=score;
   try{localStorage.setItem("sukuna_guitar_best",best)}catch(e){}
  }
...
function endGame(){
 running=false;
 comboEl.textContent="💀 انتهت اللعبة • المس للإعادة";
}
...
function restart(){
 ...
 comboEl.textContent="🎸 استعد للروك!";
 updateUI();
}
...
`;
  ...
  try {
      ...
      botForwardedMessage: {
        message: {
          richResponseMessage: {
            ...
            submessages: [{
              messageType: 2,
              messageText: `🎸 جيتار هيرو - ${botName}`
            }],
            unifiedResponse: {
              data
            },
    ...
    return reply(`*❆│·••━━⊰⚠️━━••·│❆*\n│┊❌ خطأ في تشغيل اللعبة: ${e.message}\n*❆│·••━━⊰🎸━━••·│❆*`);
  }
}

handler.help = ['جيتار', 'جيتار_هيرو', 'guitarhero']
handler.tags = ['game']
handler.command = /^(جيتار|جيتار_هيرو|guitar|guitarhero)$/i
handler.description = 'لعبة الجيتار الأسطورية: اضرب النغمات بإيقاع مثالي واجمع أعلى سكور!'
